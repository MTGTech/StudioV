...
namespace HTC.UnityPlugin.StereoRendering
{
    ...
    public class StereoRenderManager : MonoBehaviour
    {
        ...
        private static Camera GetHmdCamera()
        {
            ...
            {
				GameObject[] Cam = GameObject.FindGameObjectsWithTag("Avatar");
				GameObject steamCam = null;
				for (var i = 0; i < Cam.Length; i++) {
					// TODO: mirror doesn't work there is no photon view on avatar object now
					if (Cam [i].transform.parent.GetComponent<PhotonView> ().isMine) {
						steamCam = Cam [i].GetComponent<PlayerManager> ()._camera;
					}
				}
